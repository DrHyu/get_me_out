...
const HiltContainer = styled.div`
  ...
  ${({ isOn }) => !isOn && `animation: wiggle 5s linear infinite ;`}
...
const FlameGlow = styled.div`
  ...
  animation: glow ${animationtime} ease-in infinite alternate;
  /* animation: growOnStart 3s ease-in 1;

  @keyframes growOnStart {
    0% {
      width: 0px;
      height: 0px;
    }
...
function Torch({ size = 300, glowCircle = 400, forwardedRef, isOn }) {
  return (
    <TorchSyle>
      {isOn && <FlameGlow glowCircle={glowCircle} />}
      {isOn && (
